...
4. **Audio** (`audio/`) is a sampled piano engine. `pianoSamples.js` owns the sample manifest and renders the sample set; `pianoAudio.js` owns the voice manager and the shared output bus. See [Audio engine](#audio-engine).
...
The piano is played back from samples rather than synthesized per note.

|                    |                                                         |
| ------------------ | ------------------------------------------------------- |
| Source             | Generated by this project (`src/audio/pianoSamples.js`) |
| Licence            | Same as the repository; no third-party audio is bundled |
| Root samples       | 18, every 5 semitones from A0 to C8                     |
| Velocity layers    | 3 (soft / medium / forte) = 54 buffers                  |
| Shipped asset size | 0 bytes — samples are rendered at runtime               |
| Decoded memory     | ~24 MB, rendered in ~0.8 s of CPU                       |
| Polyphony          | 64 voices                                               |

**Why generated.** Redistributable recorded piano libraries carry attribution
terms and tens of megabytes of binaries. Rendering the set instead keeps the
repository licence-clean and adds nothing to the download, while still giving
genuine sampled playback: a partial series with real string inharmonicity,
per-partial decay rates, the piano's double decay, unison beating tuned to a
fixed beat rate, and a hammer noise transient — detail far too expensive to
synthesize per voice in real time.

**Mapping.** Each MIDI note picks its nearest root sample (never more than
±2 semitones) and plays it at `2^((midi - rootMidi) / 12)`. Velocity picks a
layer and drives a per-voice low-pass, so brightness moves continuously and
layer boundaries are not heard as steps.

**Using recorded samples instead.** The manifest is data. Give each entry a
`url` in `createSampleManifest()` and the loader fetches and decodes that file
instead of rendering it — resolved through `import.meta.env.BASE_URL`, so
GitHub Pages paths keep working. Nothing else changes.

**Fallback.** Until a note's sample exists, and if rendering ever fails, that
note plays a small oscillator voice so no keypress is silent. A DEV-only
console warning fires when the fallback is used.
